...
properties([parameters([
  choice(choices: ['us-east-1', 'us-east-2', 'us-west-1', 'us-west-2'], description: 'Pick region ', name: 'region')
  ])
  ])
...
podTemplate(cloud: 'kubernetes', label: 'packer', showRawYaml: false, yaml: template) {
    node("packer"){
        container("packer"){
            ...
            stage("Git Clone"){
                git branch: 'main', url: 'https://github.com/Nurs1996/jenkins-packer.git'
            }
            stage("Packer"){
